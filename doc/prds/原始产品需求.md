...
#### 1.2 项目目标
*   通过 AI 自动生成规范的 Git Commit Message。
...
*   **需求描述**：用户在执行 `git commit` 命令时，可以通过添加 `--ai` 参数，让工具自动生成 Commit Message。
*   **实现逻辑**：
    1.  当用户执行 `git commit --ai` 时，工具捕获该命令。
    2.  工具执行 `git diff --staged`（或类似命令）获取已暂存的代码变更内容。
    3.  将获取到的代码变更信息发送给配置好的人工智能服务（如 OpenAI API）。
    4.  AI 服务根据代码变更生成符合规范的 Commit Message。
    5.  工具使用 AI 生成的 Message 执行 `git commit -m "<generated_message>"`。
*   **相关配置**：AI 服务地址、API Key、模型选择等（详见配置管理章节）。
*   **命令行示例**：`git commit --ai -m "feat: add user login feature"` (用户提供的原始信息可以作为 AI 生成的补充或参考)
...
#### 4.1 AI 辅助 Commit (`git commit --ai`) 流程

```mermaid
graph TD
    A[用户执行: git commit --ai] --> B{工具拦截命令};
    B --> C[执行 'git diff --staged'];
    C --> D[捕获暂存区代码变更];
    D --> E[将变更信息发送至配置的 AI 服务];
    E --> F[AI 服务生成 Commit Message];
    F --> G[工具接收 Commit Message];
    G --> H[执行 'git commit -m "AI 生成的 Message"'];
    H --> I[Commit 完成];
```
